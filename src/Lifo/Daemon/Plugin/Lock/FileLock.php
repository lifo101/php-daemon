...
class FileLock extends AbstractLock
{
    ...
    private function flock()
    {
        ...
        if (!flock($this->lock, LOCK_EX | LOCK_NB)) {
            ...
            if ($pid && $pid != $this->pid) {
                throw new CleanErrorException(sprintf("PID is locked by another process PID=%s FILE=%s", $pid ?: 'unknown', $this['file']));
            }
...
//            throw new CleanErrorException(sprintf("Could not obtain lock FILE=%s", $this['file']));
    ...
    protected function getTTL()
    {
        ...
        return (($mtime ? $mtime : time()) + $this['ttl']) - time();
    }
        ...
        if (!@is_writable($path)) {
            if (!@file_exists($path) && $this['create_path']) {
                if (!@mkdir($path, 0777, true)) {
                    new CleanErrorException("PID file path \"$path\" could not be created");
                }
            } else {
