...
//                $this->purge();
    ...
    {
        return $this->sem ? sem_acquire($this->sem) : false;
    }
    ...
    {
        return $this->sem ? @sem_release($this->sem) : false;
    }

    public function get($msgType, $blocking = false, &$msg = null)
    {
        $blocking = $blocking ? 0 : MSG_IPC_NOWAIT;
        $type = $err = null;
        msg_receive($this->queue, $msgType, $type, $this->size, $msg, true, $blocking, $err);
...
//            throw new \Exception(__METHOD__ . " Failed. Could not decode message: " . json_encode($msg));
    ...
    {
        $this->lock();
        $id = $id instanceof Call ? $id->getId() : $id;
        if ($this->shm && shm_has_var($this->shm, $id)) {
            @shm_remove_var($this->shm, $id);
        }
        $this->unlock();
    }
     ...
     * @param $try
     */
    private function getDelay($delay, $try)
    {
        return $delay * pow(2, min(max($try, 1), 8)) - $delay;
    }

    private function handleIpcError($error, $try = 1)
    {
        switch ($error) {
            case 0: // success
